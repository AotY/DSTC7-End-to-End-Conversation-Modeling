...
class Dataset:
    ...
    def read_data(self):
        ...
        _data_dict_path = os.path.join(self.config.save_path, '_data_dict.4.%s_%s.pkl' % (self.config.c_min, self.config.c_max))
    ...
    def save_ground_truth(self, task):
        enc_type = self.config.enc_type
        ground_truth_path = os.path.join(self.config.save_path, 'ground_truth/%s.4.ground_truth.txt' % (
            task
        ))
