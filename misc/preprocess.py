...
def save_data_to_pair(opt, conversations, responses, conversation_ids, hash_values, response_scores, dialogue_turns, filename):
    '''Save data in pair format.'''
    save_file = open(os.path.join(opt.save_path, filename),
                     'w', encoding='utf-8')
    for conversation_id, conversation, response, hash_value, score, turn in zip(conversation_ids, conversations, responses, hash_values, response_scores, dialogue_turns):
        save_file.write('%sSPLITTOKEN%sSPLITTOKEN%sSPLITTOKEN%sSPLITTOKEN%sSPLITTOKEN%s\n' % (conversation_id, ' '.join(conversation), ' '.join(response), hash_value, str(score), str(turn)))
