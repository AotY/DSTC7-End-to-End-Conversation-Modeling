"""
dialogue metrics
https://github.com/guxd/DialogWAE/blob/master/experiments/metrics.py
"""
...
class Metrics:
    ...
    def embedding(self, seqs):
        ...
        # convert to 1-d indexes [(batch_sz*seqlen)]
        seqs = np.reshape(seqs, (-1))
        embs = self.word2vec[seqs]  # lookup [(batch_sz*seqlen) x emb_sz]
        # recover the shape [batch_sz x seqlen x emb_sz]
        embs = np.reshape(embs, (batch_size, seqlen, -1))
        return embs

    # embs: [batch_size x seq_len x emb_size]  lens: [batch_size]
    def extrema(self, embs, lens):
        """
    ...
    def sim_bow(self, pred, pred_lens, ref, ref_lens):
        ...
        # [batch_sz_pred x batch_sz_ref]
        bow_extrema = cosine(ext_emb_pred, ext_emb_ref)

        # Calculate mean over seq
        avg_emb_pred = self.mean(emb_pred, pred_lens)
        avg_emb_ref = self.mean(emb_ref, ref_lens)
        # [batch_sz_pred x batch_sz_ref]
        bow_avg = cosine(avg_emb_pred, avg_emb_ref)
    ...
    def div_distinct(self, seqs, seq_lens):
        ...
        for b in range(batch_size):
            unigrams = Counter([tuple(seqs[b, i:i+1])
                                for i in range(seq_lens[b])])
            bigrams = Counter([tuple(seqs[b, i:i+2])
                               for i in range(seq_lens[b]-1)])
            intra_dist1[b] = (len(unigrams.items())+1e-12)/(seq_lens[b]+1e-5)
            intra_dist2[b] = (len(bigrams.items())+1e-12) / \
                (max(0, seq_lens[b]-1)+1e-5)

            unigrams_all.update([tuple(seqs[b, i:i+1])
                                 for i in range(seq_lens[b])])
            bigrams_all.update([tuple(seqs[b, i:i+2])
                                for i in range(seq_lens[b]-1)])
