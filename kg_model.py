    ...
    def h_forward(self, h_encoder_inputs, batch_size):
        ...
        if self.turn_type == 'concat' or self.turn_type == 'none':
            ...
            for h_encoder_input in h_encoder_inputs:
                ...
                if len(h_encoder_input) == 0:
                    session_encoder_output = torch.zeros((1, 1, self.hidden_size), device=self.device)
                else:
                    for ids in h_encoder_input:
                        simple_encoder_hidden_state = self.simple_encoder.init_hidden(1, self.device)
                        h_encoder_outputs, h_encoder_hidden_state = self.simple_encoder(ids.view(-1, 1), simple_encoder_hidden_state)
                        # session update
                        session_encoder_outputs, session_encoder_hidden_state = self.session_encoder(h_encoder_outputs[-1].unsqueeze(0), session_encoder_hidden_state)

                    session_encoder_output = session_encoder_outputs[-1].unsqueeze(1)

                h_encoder_outputs.append(session_encoder_output)
            h_encoder_outputs = torch.cat(h_encoder_outputs, dim=1) # [1, batch_size, hidden_size]
