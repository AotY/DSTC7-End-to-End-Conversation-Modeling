...
class BaseRNNEncoder(nn.Module):
    ...
    def init_h(self, batch_size=None, hidden=None):
        ...
        if self.use_lstm:
            return (to_device(torch.zeros(self.num_layers*self.num_directions,
                                      batch_size,
                                      self.hidden_size)),
                    to_device(torch.zeros(self.num_layers*self.num_directions,
                                      batch_size,
                                      self.hidden_size)))
        else:
            return to_device(torch.zeros(self.num_layers*self.num_directions,
...
class EncoderRNN(BaseRNNEncoder):
    ...
    def forward(self, inputs, input_length, hidden=None):
        """
        Args:
            inputs : [num_sentences, max_seq_len]
            input_length (Variable, LongTensor): [num_sentences]
        Return:
            outputs : [max_source_length, batch_size, hidden_size]
...
class ContextRNN(BaseRNNEncoder):
    def __init__(self, input_size, context_size,
                 rnn=nn.GRU, num_layers=1, dropout=0.0,
