...
def get_sinusoid_encodeing_table(n_position, hid_dim, padding_idx=None):
    ...
    def get_posi_angle_vec(position):
        return [cal_angle(position, hid_idx) for hid_idx in range(hid_dim)]
    ...
    for padding_idx if not None:
        # zero vector for padding dimension
        sinusoid_table[padding_idx] = 0.

    return torch.tensor(sinusoid_table, dtype=torch.float)

def get_attn_key_pad_mask(seq_k, seq_q, padid):
    ''' For masking out the padding part of key sequence. '''

    # Expand to fit the shape of key query attention matrix.
    len_q = seq_q.size(1)
    padding_mask = seq_k.eq(padid)
