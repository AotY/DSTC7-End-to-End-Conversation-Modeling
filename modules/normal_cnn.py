...
class NormalCNN(nn.Module):
    ...
    def forward(self, inputs, lengths=None):
        ...
        # conv
        conv2d1_output = self.conv2d1(embedded) # [batch_size, 512, 32, 1]
        conv2d1_output = F.relu(conv2d1_output)
        conv2d1_output = self.maxpool3(conv2d1_output) # [batch_size, 512, 29, 1]
        print('conv2d1_output: ', conv2d1_output.shape)

        conv2d2_input = conv2d1_output.transpose(1, 3) # [batch_size, 1, 29, 512]
        conv2d2_output = self.conv2d2(conv2d2_input) # [batch_size, 256, 27, 1]
        conv2d2_output = F.relu(conv2d2_output)
        conv2d2_output = self.maxpool3(conv2d2_output) # [batch_size, 256, 25, 1]
        print('conv2d2_output: ', conv2d2_output.shape)

        conv2d3_input = conv2d2_output.transpose(1, 3) # [batch_size, 1, 25, 256]
        conv2d3_output = self.conv2d3(conv2d3_input) # [batch_size, 128, 22, 1]
        conv2d3_output = F.relu(conv2d3_output)
        conv2d3_output = self.maxpool4(conv2d3_output) # [batch_size, 128, 19, 1]

        conv2d4_input = conv2d3_output.transpose(1, 3) # [batch_size, 1, 19, 128]
        conv2d4_output = self.conv2d4(conv2d4_input) # [batch_size, 256, 16, 1]
        conv2d4_output = F.relu(conv2d4_output)
        conv2d4_output = self.maxpool4(conv2d4_output) # [batch_size, 256, 13, 1]

        conv2d5_input = conv2d4_output.transpose(1, 3) # [batch_size, 1, 13, 256]
        conv2d5_output = self.conv2d5(conv2d5_input) # [batch_size, 512, 9, 1]
        conv2d5_output = F.relu(conv2d5_output)
        conv2d5_output = self.maxpool5(conv2d5_output) # [batch_size, 512, 5, 1]

        conv2d6_input = conv2d5_output.transpose(1, 3) # [batch_size, 1, 5, 512]
        conv2d6_output = self.conv2d6(conv2d6_input) # [batch_size, 1024, 3, 1]
