...
class LuongAttnDecoder(nn.Module):
    def __init__(self, config, embedding):
        ...
        self.f_linearA = nn.Linear(
            self.embedding_size, config.hidden_size + config.latent_size)

        self.f_linearC = nn.Linear(
            self.embedding_size, config.hidden_size + config.latent_size)

        self.f_concat_linear = nn.Linear(config.hidden_size * 2, config.hidden_size)
        init_linear_wt(self.f_concat_linear)
        ...
        if config.latent_size > 0:
            self.latent_linear = nn.Linear(
                config.hidden_size + config.latent_size, config.hidden_size)
    ...
    def f_forward(self, output, f_encoder_outputs, f_encoder_lengths=None):
        ...
        mix = torch.bmm(weights, fV).transpose(
            0, 1)  # [1, batch_size, hidden_size]

        # [1, batch_size, 2 * hidden_size]
        output = torch.cat((mix, output), dim=2)
